...
class DetectionTrial(Trial):
    ...
    def __init__(self, session, trial_nr, phase_durations, phase_names,
        ...
        self.mySound = sound.Sound('noise.wav', stereo=True)
    ...
    def create_trials(self):
        ...
        for i, parameters in enumerate(trial_parameters):
            ...
            trial = DetectionTrial(
                session=self,
                trial_nr=i,
                phase_durations=(intro_dur, 1, 5, np.random.uniform(2.5,3.5,1)),
                timing='seconds',
...
if __name__ == '__main__':
    ...
    awake = input('How awake are you [1-7]? ')
    ...
    print(contrast)
    my_sess = GaborSession(output_str='{}_{}_{}'.format(subject_nr, block_nr, dt), output_dir='data/2_yesno', 
                            settings_file=settings, task='yes_no', n_trials=360, contrast=contrast, block_type=block_type, awake=awake)
    my_sess.run()
    time.sleep(1) # Sleep for 1 second

    # analyze:
    filename = glob.glob(op.join('data', '2_yesno', '{}_{}_{}*events.tsv'.format(subject_nr, block_nr, dt)))[0]
