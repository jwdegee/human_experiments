...
class TestEyetrackerSession(PylinkEyetrackerSession):
    ...
    def create_trials(self, timing='seconds'):
        ...
        def compute_post_target(target, prev_target):
            target = np.array(target)
            prev_target = np.array(prev_target)
            print(prev_target.type,target.type)
            post_target = target+(target-prev_target)*0.1
        ...
        for n in range(1,self.n_trials):
            post_targets.append(compute_post_target(targets[n], post_targets[n-1])) 
