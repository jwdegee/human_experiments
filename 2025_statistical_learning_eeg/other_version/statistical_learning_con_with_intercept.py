...
class TestTrial(Trial):
    ...
    def __init__(self, session, trial_nr, phase_durations, parameters, **kwargs):
        ...
        if 'freq' in self.parameters:
            ...
            freq_start = 1414.21      # first tone (half-octave above 1000 Hz)
            freq_target = self.parameters['freq']  # second tone, from your code
            ...
            # 0.2s first tone at 1414.21 Hz
            t1 = np.linspace(0, dur_tone1, int(fs * dur_tone1), endpoint=False)
            tone1 = np.sin(2 * np.pi * freq_start * t1)

            # 0.1s silence
            silence = np.zeros(int(fs * dur_silence))

            # 0.2s second tone at chosen freq
            t2 = np.linspace(0, dur_tone2, int(fs * dur_tone2), endpoint=False)
            tone2 = np.sin(2 * np.pi * freq_target * t2)

            # concatenate
            waveform = np.concatenate([tone1, silence, tone2])

            # make PsychoPy sound
            
            self.snd = sound.Sound(waveform, sampleRate=fs, hamming=True, secs=0.5, octave=4, stereo=True)
...
if __name__ == '__main__':
    ...
    session = TestEyetrackerSession(output_str='{}_{}_{}'.format(subject_nr, block_nr, dt),
                                    output_dir='data/',
                                    eyetracker_on=True, n_trials=500, settings_file=settings)
