...
class GaborSession(Session): #Run session (or self) with multiple trials

    # initialize parent class that will contain several trials!
    def __init__(self, output_str, output_dir=None, settings_file=None, task='diagonal_cardinal', n_trials=1, awake=0):
    ...
    def run(self):
        ...
        conditions = ['normal', 'normal', 'normal', 'normal', 'normal']
        ...
        for i in range(self.n_trials):
            ...
            if i == 0:
                phase_durations=[30,  1, 0.250, 0.250, 0.250, 0.250, 0.250, 0.250, 0.250, 0.250, 5, 0.25, np.random.uniform(1,2,1)]
            else:
                phase_durations=[0.1, 1, 0.250, 0.250, 0.250, 0.250, 0.250, 0.250, 0.250, 0.250, 5, 0.25, np.random.uniform(1,2,1)]
...
if __name__ == '__main__':
    ...
    my_sess = GaborSession(output_str='{}_{}_{}'.format(subject_nr, block_nr, dt), output_dir='data/0_practise', settings_file=settings, task='diagonal_cardinal', n_trials=80, awake=awake) #Set and run a session
    my_sess.run()
    time.sleep(1) # Sleep for 1 second
   
    # analyze + data acquisition (file with the data)
    filename = glob.glob(op.join('data', '0_practise', '{}_{}_{}*events.tsv'.format(subject_nr, block_nr, dt)))[0]
